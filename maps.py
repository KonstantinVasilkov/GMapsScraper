...
class GMapsScraper:
    ...
    def arg_parser(self):
        parser = ArgumentParser(
            description="Command Line Google Map Scraper by Abdul Moez"
        )
        ...
        parser.add_argument(
            "-q",
            "--query-file",
            help="Path to query file (default: ./queries.txt)",
            type=str,
            default="./queries.txt",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-u",
            "--unavailable-text",
            help='Replacement text for unavailable information (default: "Not Available")',
            type=str,
        ...
        parser.add_argument(
            "-bw",
            "--browser-wait",
            help="Browser waiting time in seconds (default: 15)",
            type=int,
            default=15,
        )
        parser.add_argument(
            "-se",
            "--suggested-ext",
            help="Suggested URL extensions to try (can be specified multiple times)",
            action="append",
            default=[],
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-d",
            "--driver-path",
            help="Path to Chrome driver (if not provided, it will be downloaded)",
            type=str,
            default="",
        )
        ...
        parser.add_argument(
            "--help-query-file",
            action="store_true",
            help="Get help for specifying the query file",
        )
        parser.add_argument(
            "--help-limit",
            action="store_true",
    ...
    def print_query_file_help():
        print(
            "The query file should contain a list of search queries, each query on a separate line."
        )
        print("For example:")
    ...
    def print_driver_path_help():
        print(
            "If you have a specific Chrome driver path, you can provide it using this option."
        )
        print(
            "If not provided, the script will attempt to download the driver automatically."
        )
        print(
            "You can download a compatible driver from https://chromedriver.chromium.org/downloads."
        )
        sys.exit(0)
    ...
    def scrape_maps_data(self):
        ...
        if not self._args.driver_path:
            try:
                ...
                print(
                    "[-] Not able to download the driver which is capable with your browser."
                )
                print(
                    "[INFO] Head to this site (https://chromedriver.chromium.org/downloads)"
